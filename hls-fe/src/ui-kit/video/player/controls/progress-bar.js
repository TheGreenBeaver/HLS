...
import { UPD_RATES, VIDEO_DATA_SHAPE } from '../util';
...
function ProgressBar({ loadedTimeRanges, totalDuration, currentTimeData }) {
  ...
  const throttleControllerRef = useRef(null);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      throttleControllerRef.current = setTimeout(() => setThrottleHasPassed(true), UPD_RATES.currentTime);
    };
  }, [isSeeking])
  ...
  function getThumbPosition() {
    ...
    return `${isSeeking || !throttleHasPassed ? tempThumbPosition : currentTime / totalDuration * 100}%`;
  }
