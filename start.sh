...
docker-compose kill -s SIGHUP insight-proxy

# Save current crontab to temp file
crontab -l > cron.temp
# Add cron job for renewal
echo "0 0 */1 * * docker-compose run insight-certbot renew && docker-compose kill -s SIGHUP insight-proxy > /dev/null" >> cron.temp
# Set new crontab
crontab cron.temp
# Remove temp file
rm cron.temp